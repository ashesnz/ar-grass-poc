...
def getGrassId(id: str, input_images: str, skip_models_in_directory: str, model_suffix: str):
    """Gets grass ID, filling in missing zero prefix when necessary"""
...
class Grass:
    """Grass data."""
    def __init__(self, image: GrassImage, length_m: float, width_m: float):
        self.image = image
...
class Generator:
    ...
    def generate(self):
        ...
        with open(self.args.input_csv, newline='') as csvfile:
            ...
            num_written = 0
            for row in reader:
                grass_id = getGrassId(row['ID'], self.args.input_images,
                                      self.args.skip_models_in_directory, suffix)
                image = GrassImage(grass_id, self.args.input_images)
                model_filename = f"{grass_id}.{suffix}"
                ...
                grass = Grass(image, float(row['L cm']) / 100, float(row['W cm']) / 100)
                if grass.image.getOriginalFilename() is not None:
                    gltf = grass.getGLTF(store_image_in_blob=self.args.save_as_glb)
                    num_written += 1
                    if not self.args.save_as_glb:
                        print(f"[{num_written}] Writing {output_model_file}")
                        gltf.save(output_model_file)
                        image_file = os.path.join(self.args.output_models, grass.image.getLocalFilename())
                        print(f"[{num_written}] Copying {image_file}")
                        shutil.copyfile(
                            grass.image.getOriginalFilename(),
                            image_file)
                    else:
                    ...
                    print(f"WARNING: Skipping grass without image file. Grass ID={grass_id}")
            print("\nSuccess!\n")
