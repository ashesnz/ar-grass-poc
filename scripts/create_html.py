...
def main():
    ...
    select_lines = [_SELECT_LINE.format(model_id=f, selected_str=('selected' if f == _DEFAULT_GRASS_ID else '')) for f in model_ids]

    with open(args.output_html, "w") as file1:
        # Writing data to a file
        file1.write(_PREAMBLE)
        file1.writelines(select_lines)
        file1.write(_POSTAMBLE.format(model_id=_DEFAULT_GRASS_ID))
